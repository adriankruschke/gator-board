...
import { playClick, playDraw, playElderSign, playHorn } from './sound';
...
function pullTokens(count: number) {
  ...
  // The auto-fail and the elder sign get their own sound instead of the usual pull sound.
  else if (pulled.includes('auto_fail')) playHorn();
  else if (pulled.includes('elder_sign')) playElderSign();
  else playDraw();
}
