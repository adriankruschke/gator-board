...
- **Reset campaign** (in the log) restarts the same investigator: trackers back to their defaults, log erased, session timer restarted. The chaos bag is left exactly as it is — contents, bless and curse, sealed and drawn tokens all survive.
  - The confirmation also offers "Reset the chaos bag too", which asks a second time before rebuilding the bag for the campaign and difficulty it is set to.
  - Choosing an investigator on the start page always starts a fresh bag.
- **Session timer** (in the campaign log): how long ago the session started, and how long the board has actually been open. Neither is counted by a ticking clock — the start is a stored timestamp and foreground time accrues between `visibilitychange` events, so sleeping or backgrounding the iPad cannot make it drift.
  - Battery: no timer runs while the board is hidden, and none while the log is closed. The only recurring work is a 60-second save while the board is on screen, so a force-quit cannot lose more than a minute of play time. Everything else rides on the saves that already happen when you change a tracker or draw a token, plus `visibilitychange` and `pagehide`.
- The session is saved to first-party cookies (split into chunks) and restored on load. Older saves without an investigator load as Harvey Walters. Cookie names carry the app's path (`gb_gator_board_*`): `document.cookie` hides which path a cookie came from, so the root-scoped `gb_*` cookies an early build wrote would otherwise shadow the current ones. Those are migrated and deleted on first load.
